...
export default function LiveCamera({ 
  ...
  return (
    <div className="group relative bg-white rounded-2xl overflow-hidden shadow-xl lift-on-hover">
      ...
      {/* Refresh Button (only for static images) */}
      {!isIframe && (
        <button
      ...
      <div className="relative aspect-video bg-gradient-to-br from-alpine-blue/5 to-sunset-orange/5">
        ...
        {/* Gradient Overlay for Text (not for iframe) */}
        {!isIframe && (
      ...
      {/* Info Below for Iframe */}
      {isIframe && (
        <div className="p-6 bg-white">
          <h3 className="font-display text-2xl text-alpine-blue mb-1">{name}</h3>
          {description && (
            <p className="text-mountain-night/70 text-sm">{description}</p>
          )}
        </div>
      )}
